            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="w-full">
                    <h2 class="text-xl sm:text-2xl font-bold text-gray-800 dark:text-gray-100">
                        Materia - Periodo - Paralelo
                    </h2>
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                        Administra los módulos asignados por periodo y paralelo.
                    </p>

                    <div class="mt-4 flex flex-col sm:flex-row gap-3">
                        <div class="w-full">
                            <input wire:model.live="search"
                                class="w-full rounded-lg shadow-sm bg-gray-50 dark:bg-gray-800/70 dark:text-gray-200 text-gray-800
                                   border border-gray-300 dark:border-gray-700
                                   focus:border-lime-500 focus:ring focus:ring-lime-200 dark:focus:ring-lime-900/40
                                   px-4 py-2.5 text-sm"
                                placeholder="Buscar por materia, paralelo, periodo, fechas o estado...">
                        </div>
                ...
                <div
                    ...
                    <div class="mt-4 grid grid-cols-2 gap-3 text-sm">
                        ...
                        <div class="rounded-lg bg-gray-50 dark:bg-gray-800/60 p-3">
                            ...
                            <p class="font-semibold text-gray-800 dark:text-gray-100">
                                {{ \Carbon\Carbon::parse($registro->fecha_fin)->format('Y-m-d') }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="mt-4 flex items-center justify-end gap-2">
                        ...
                        <form
                            action="{{ route('administracion.administrativa.materia_periodo_paralelo.destroy', $registro) }}"
                            method="POST" onsubmit="return confirm('¿Seguro deseas eliminar este módulo?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                class="inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-semibold
            ...
            @empty
                <div
                    ...
                    <p class="text-gray-600 dark:text-gray-300">
                        No existe registro que coincida con el parámetro de búsqueda.
                    </p>
                </div>
            @endforelse
        </div>
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-gray-50 dark:bg-gray-800/60">
                        <tr class="text-gray-700 dark:text-gray-200">
                            <th class="w-16 px-4 py-3 text-center font-bold">ID</th>
                            <th class="px-4 py-3 text-left font-bold">Materia</th>
                            <th class="px-4 py-3 text-left font-bold">Periodo</th>
                            <th class="px-4 py-3 text-left font-bold">Paralelo</th>
                            <th class="w-32 px-4 py-3 text-center font-bold">Inicio</th>
                            <th class="w-32 px-4 py-3 text-center font-bold">Fin</th>
                            <th class="w-28 px-4 py-3 text-center font-bold">Estado</th>
                            <th class="w-28 px-4 py-3 text-center font-bold">Opciones</th>
                        </tr>
                    </thead>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-3">
                                        ...
                                        <form class="inline-block"
                                            action="{{ route('administracion.administrativa.materia_periodo_paralelo.destroy', $registro) }}"
                                            method="POST"
                                            onsubmit="return confirm('¿Seguro deseas eliminar este módulo?')">
                                            @csrf
                                            @method('DELETE')
