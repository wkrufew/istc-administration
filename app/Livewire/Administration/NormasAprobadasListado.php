...
class NormasAprobadasListado extends Component
{
    use WithPagination;
    ...
    {
        $documents = NormasAprobada::where('title', 'LIKE', '%' . $this->search . '%')
            ->latest('id')
            ->paginate(10);

        return view('livewire.administration.normas-aprobadas-listado', compact('documents'));
    }
    ...
    {
        $document = NormasAprobada::find($document);
