            ...
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="bg-gray-900 dark:bg-black text-white text-xs uppercase tracking-wide">
                        ...
                        <th class="px-4 py-3 text-left">Fechas Matrículas</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800 bg-white dark:bg-gray-900">
                    @forelse ($periodos as $periodo)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/60 transition">
                            ...
                            <td class="px-4 py-3">
                                <span
                                    class="px-2 py-1 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200 text-xs font-semibold">
                                    {{ $periodo->code }}
                                </span>
                            </td>
                            ...
                            <td class="px-4 py-3 text-gray-700 dark:text-gray-200">
                                <div class="flex flex-col">
                                    <span class="font-semibold text-gray-800 dark:text-gray-100">
                                        {{ $periodo->fecha_inicio->isoFormat('D [de] MMM [de] Y') }}
                                    </span>
                                    <span class="text-xs text-gray-500 dark:text-gray-400">
                                        hasta {{ $periodo->fecha_fin->isoFormat('D [de] MMM [de] Y') }}
                                    </span>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3 text-gray-700 dark:text-gray-200">
                                <div class="flex flex-col">
                                    <span class="font-semibold text-gray-800 dark:text-gray-100">
                                        {{ $periodo->fecha_limite_matricula->isoFormat('D [de] MMM [de] Y') }}
                                    </span>
                                    <span class="text-xs text-gray-500 dark:text-gray-400">
                                        pago: {{ $periodo->fecha_limite_pago->isoFormat('D [de] MMM [de] Y') }}
                                    </span>
                                </div>
                            </td>
            ...
            @forelse ($periodos as $periodo)
                <div
                    ...
                    <div class="mt-3 text-sm text-gray-700 dark:text-gray-200 space-y-2">
                        <p>
                            <span class="font-semibold">Periodo:</span><br>
                            {{ $periodo->fecha_inicio->isoFormat('D [de] MMM [de] Y') }}
                            <span class="text-gray-500 dark:text-gray-400">→</span>
                            {{ $periodo->fecha_fin->isoFormat('D [de] MMM [de] Y') }}
                        </p>

                        <p>
                            <span class="font-semibold">Matrícula:</span><br>
                            {{ $periodo->fecha_limite_matricula->isoFormat('D [de] MMM [de] Y') }}
                        </p>

                        <p>
                            <span class="font-semibold">Pago:</span><br>
                            {{ $periodo->fecha_limite_pago->isoFormat('D [de] MMM [de] Y') }}
                        </p>
                    </div>
                    ...
                    <div class="mt-4 flex gap-3">
                        ...
                        <form action="{{ route('administracion.administrativa.periodos.destroy', $periodo) }}"
