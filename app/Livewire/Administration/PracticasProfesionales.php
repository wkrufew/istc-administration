...
class PracticasProfesionales extends Component
{
    ...
    {
        return PracticaPreprofesional::with(['estudiante', 'carrera'])
    ...
    // ABRIR MODAL EDITAR
    // =========================================================================
    public function abrirModalEditar(int $practicaId): void
    {
        $practica = PracticaPreprofesional::with(['estudiante', 'carrera'])->find($practicaId);
        if (! $practica) return;
        ...
        $this->estudianteId     = $practica->user_id;
        $this->estudianteNombre = $practica->estudiante?->name ?? '';
        $this->carreraId        = $practica->carrera_id;
        $this->carreraNombre    = $practica->carrera?->name ?? '';

        // Campos
        $this->empresa         = $practica->empresa;
        $this->sector          = $practica->sector ?? '';
        $this->direccion       = $practica->direccion_empresa ?? '';
        $this->tutorEmpresa    = $practica->tutor_empresa;
        $this->cargoTutor      = $practica->cargo_tutor_empresa ?? '';
        $this->telefono        = $practica->telefono_empresa ?? '';
        $this->email           = $practica->email_empresa ?? '';
        $this->cargoEstudiante = $practica->cargo_estudiante;
        $this->actividades     = $practica->actividades_realizadas ?? '';
        $this->fechaInicio     = $practica->fecha_inicio?->format('Y-m-d') ?? '';
        $this->fechaFin        = $practica->fecha_fin?->format('Y-m-d') ?? '';
        $this->totalHoras      = (string) ($practica->total_horas ?? '');
        $this->nota            = (string) ($practica->nota ?? '');
        $this->estado          = $practica->estado;
        $this->observaciones   = $practica->observaciones ?? '';

        // Paths actuales de documentos
        $this->cartaAceptacionPath = $practica->carta_aceptacion_path;
        $this->informeFinalPath    = $practica->informe_final_path;
        $this->certificadoPath     = $practica->certificado_empresa_path;
    ...
    {
        if ($this->sinPermiso('gestionar_practicas_preprofesionales')) return;
        ...
        try {
            ...
            if ($this->modoEdicion) {
                ...
                PracticaPreprofesional::create($datos);
    ...
    // ELIMINAR
    // =========================================================================
    public function eliminar(int $practicaId): void
    {
        try {
            $practica = PracticaPreprofesional::findOrFail($practicaId);

            // Eliminar archivos del storage
            if ($practica->carta_aceptacion_path)
                Storage::disk('public')->delete($practica->carta_aceptacion_path);
            if ($practica->informe_final_path)
                Storage::disk('public')->delete($practica->informe_final_path);
            if ($practica->certificado_empresa_path)
    ...
    {
        return view('livewire.administration.practicas-profesionales', [
